...
const Dashboard = ({ userEmail: propUserEmail }) => { 
    ...
    // Calculate performers statistics 
...
const performersData = Object.values(performersStats)
  .sort((a, b) => (b.completed / b.total) - (a.completed / a.total));
    ...
    // Format deadline dates for display 
    ...
    const timePerTaskData = filteredTasks.map(task => ({ name: task.title?.substring(0, 15) || 'Задача', time: Number(task.work_duration) || 0 }));
...
{/* New Deadline Chart and Performers Table */}
                            ...
                            <Grid item xs={12} md={6}>
                                <ChartCard>
                                    <CardContent>
                                        ...
                                        {performersData.length > 0 ? (
                                            <TableContainer component="div" style={{ maxHeight: 400 }}>
                                                <Table stickyHeader size="small">
                                                    <TableHead>
                                                        <TableRow>
                                                            <TableCell>Исполнитель</TableCell>
                                        ...
                                        {performersData.length > 0 && (
                                            <Box mt={2}>
                                                ...
                                                <Box display="flex" gap={2} mt={1}>
                                                    <Chip 
                                                        label={`Лучший исполнитель: ${performersData[0]?.name}`}
                                                        color="success"
                                                        size="small"
                                                    />
                                                    <Chip 
                                                        label={`Выполнено задач: ${performersData.reduce((sum, p) => sum + p.completed, 0)}`}
                                                        color="primary"
                                                        size="small"
                                                    />
                                                </Box>
